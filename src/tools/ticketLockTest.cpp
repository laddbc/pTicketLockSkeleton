 ...
 * shared TicketLock.
 *
 * A thread-local randomness generator (gen) is constructed locally;
 * this makes sure it is thread safe to generate a sequence of random
 * numbers (the actual sleep values).
 *
 ...
 */
void thread_worker(const string & threadName, int loops, volatile TicketLock & ticket_lock) {
...
int main(int argc, char *argv[]) {
  ...
  for (unsigned int i = 0; i < number_of_threads; ++i) {
    ...
    threads[i] = new thread(&thread_worker, name, number_of_iterations, ref(lock));
  }
